...
function createMessage(eventType, event) {
  if (eventType === 'route_assignment') {
    ...
    return {
      type: 'interactive',
      text: '📍 Location Update Request\\n\\nCurrent position: ' + event.data.address + '\\nSpeed: ' + event.data.speed + ' km/h\\nStatus: ' + event.data.status + '\\n\\nPlease confirm your current location for route optimization.',
      buttons: [
    ...
    return {
      type: 'interactive',
      text: '🏁 Arrival at ' + event.data.geofenceName + '\\n\\nLocation: ' + event.data.geofenceName + '\\nArrival Time: ' + new Date(event.data.arrivalTime).toLocaleString() + '\\n\\nPlease confirm your arrival and loading status.',
      buttons: [
        { id: 'start_loading', text: '📦 Start Loading' },
        { id: 'report_arrival', text: '📍 Confirm Arrival' },
        { id: 'need_assistance', text: '🆘 Need Assistance' }
      ],
      timestamp: new Date().toISOString()
    };
  }
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div id="samsaraPanel" class="bg-white rounded-lg shadow-lg p-6">
                ...
                <div class="space-y-4">
                    <button onclick="sendEvent('route_assignment')" 
                            class="w-full text-left p-4 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors">
                        <div class="flex items-center">
                            <div class="w-10 h-10 bg-blue-500 text-white rounded-full flex items-center justify-center text-xl mr-3">🚚</div>
                            <div>
                                <h3 class="font-semibold text-gray-900">Route Assignment</h3>
                                <p class="text-sm text-gray-600">New delivery route assigned to driver</p>
                            </div>
                        </div>
                    </button>

                    <button onclick="sendEvent('location_update')" 
                            class="w-full text-left p-4 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 transition-colors">
                        <div class="flex items-center">
                            <div class="w-10 h-10 bg-green-500 text-white rounded-full flex items-center justify-center text-xl mr-3">📍</div>
                            <div>
                                <h3 class="font-semibold text-gray-900">Location Update</h3>
                                <p class="text-sm text-gray-600">Request current location from driver</p>
                            </div>
                        </div>
                    </button>

                    <button onclick="sendEvent('geofence_entry')" 
                            class="w-full text-left p-4 bg-purple-50 border border-purple-200 rounded-lg hover:bg-purple-100 transition-colors">
                        <div class="flex items-center">
                            <div class="w-10 h-10 bg-purple-500 text-white rounded-full flex items-center justify-center text-xl mr-3">🏁</div>
                            <div>
                                <h3 class="font-semibold text-gray-900">Geofence Entry</h3>
                                <p class="text-sm text-gray-600">Driver arrived at destination</p>
                            </div>
                        </div>
                    </button>
                </div>
                ...
                <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                    <h4 class="font-medium text-gray-900 mb-2">Last Event Data:</h4>
                    <pre id="lastEvent" class="text-xs text-gray-600 whitespace-pre-wrap font-mono">Click a button above to see event data</pre>
                </div>
            ...
            <div class="bg-white rounded-lg shadow-lg p-6">
                ...
                <p class="text-gray-600 mb-6">Driver communication via WhatsApp Business API</p>
                
                <div id="chatArea" class="min-h-96 bg-gray-50 rounded-lg p-4 mb-4">
                    <div class="text-center text-gray-500 py-8">
                        <p>No messages yet</p>
                        <p class="text-sm mt-2">Click a Samsara event to see WhatsApp messages</p>
                    </div>
                </div>

                <div class="p-3 bg-blue-50 rounded-lg">
                    <p class="text-sm text-blue-700">
                        <strong>Response Types:</strong> Button clicks, location sharing, document uploads, status confirmations
                    </p>
                </div>
            </div>
        </div>
